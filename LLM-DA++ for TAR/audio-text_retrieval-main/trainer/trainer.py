...
def train(config):
    ...
    # set up data loaders
    train_loader = get_dataloader('train', config, config.train_sample, False)
    val_loader = get_dataloader('val', config, False, False)
    ...
    for epoch in range(ep, config.training.epochs + 1):
        ...
        for batch_id, batch_data in tqdm(enumerate(train_loader), total=len(train_loader)):
            if config.train_sample:
               audios, captions, captions_aug, audio_ids, _ = batch_data
               # move data to GPU
