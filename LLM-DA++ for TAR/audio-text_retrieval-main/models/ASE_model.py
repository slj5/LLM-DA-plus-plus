...
class ASE(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.test_sample = config.test_sample
        self.test_sample_ratio = config.test_sample_ratio
        ...
        if config.text_encoder == 'bert':
            ...
            from models.TextEncoder import W2VEncoder
    ...
    def forward(self, audios, captions, captions_aug):

        if self.train_sample:
            if random.random() < self.train_sample_ratio:
                captions = captions_aug

        if self.test_sample:
            if random.random() < self.test_sample_ratio:
                captions = captions_aug
