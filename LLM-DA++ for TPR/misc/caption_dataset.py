...
class ps_eval_dataset(Dataset):
    def __init__(self, ann_root, test_sample, test_sample_P, image_root, transform, split, max_words=30):
        ann_file = os.path.join(ann_root, split + '_reid.json')
        ...
        self.test_sample_P = test_sample_P
        texts = []
        texts_ts = []
        # test_sample
        if self.test_sample:
            for ann in anns:
                ...
                for caption_ts in ann['captions_bt']:
                    texts_ts.append(caption_ts)


            for i in range(len(texts)):
                if random.random() < self.test_sample_P:
                    texts[i] = texts_ts[i]
