...
def run(config):
    ...
    output_dir = op.join(config.logger.output_dir, config.logger.dataset_name)
    logger = setup_logger('TBPS', save_dir=output_dir, if_train=config.logger.training)
    ...
    for epoch in range(config.schedule.epoch):
        ...
        if is_master():
            ...
            eval_result = test(model.module, dataloader['test_loader'], 77, config.device, config, 0)
            ...
            logger.info("Epoch {} done. Time per batch: {:.3f}[s] Speed: {:.1f}[samples/s]".format(epoch + 1,
                                                                                                   time_per_batch, train_loader.batch_size / time_per_batch))

            logger.info('Acc@1 {top1:.5f} Acc@5 {top5:.5f} Acc@10 {top10:.5f} mAP {mAP:.5f}'.format(top1=rank_1, top5=rank_5,
                                                                                   top10=rank_10, mAP=map))
    ...
    print(f"best Acc@1: {best_map} at epoch {best_epoch + 1}")


if __name__ == '__main__':

    args = get_args()
    config_path = 'config/config_orig.yaml'
