...
class nDCG:
    def __init__(self, dataset, n_queries, split, rank=25, relevance_methods=['spacy']):
        self.rank = rank
        self.relevance_methods = relevance_methods
        relevance_dir = os.path.join('outputs', 'computed_relevances')
        relevance_filenames = [os.path.join(relevance_dir, '{}-{}-{}-aug-0.8.npy'.format(dataset, split, m))
        ...
        for r in self.relevances:
            r.shape = (n_queries, -1)
        # self.relevances = []
        # for f in relevance_filenames:
        #     try:
        #         memmap_obj = np.memmap(f, dtype=np.float32, mode='r')
        #         memmap_obj.shape = (n_queries, -1)
        #         self.relevances.append(memmap_obj)
