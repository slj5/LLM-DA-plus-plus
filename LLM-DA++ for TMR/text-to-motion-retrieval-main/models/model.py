...
class MatchingModel(nn.Module):
    ...
    def compute_embeddings(self, motion, motion_len, text, text_aug, train_sample, train_sample_ration, return_all=False):
        ...
        self.train_sample_ration = train_sample_ration

        # process sentence
        # add train sample
        if self.train_sample:
            if random.random() < self.train_sample_ration:
                text = text_aug
    ...
    def forward(self, motion, motion_len, text, text_aug, train_sample, train_sample_ration, epoch=0):
        # forward the embeddings
        comp_data = self.compute_embeddings(motion, motion_len, text, text_aug, train_sample, train_sample_ration, return_all=True)
