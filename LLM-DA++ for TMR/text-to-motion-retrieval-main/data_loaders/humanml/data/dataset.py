...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer, labels_file, randomize, train_sample):
        self.opt = opt
        self.w_vectorizer = w_vectorizer
        self.max_length = 20
        self.pointer = 0
                ...
                with cs.open(pjoin(opt.text_dir, name + '.txt')) as f:
                    for line_no, line in enumerate(f.readlines()):
                        ...
                        if self.train_sample:
                            caption_aug = line_split[4]
                            tokens_aug = line_split[5].split(' ')
                        else:
                            caption_aug = caption
                            tokens_aug = tokens
                        ...
                        text_dict['caption'] = caption
                        text_dict['caption_aug'] = caption_aug
                        text_dict['tokens'] = tokens
                        text_dict['tokens_aug'] = tokens_aug
    ...
    def __getitem__(self, item):
        ...
        # Randomly select a caption
        # text_data = random.choice(text_list)
        caption, caption_aug, tokens, tokens_aug = text_data['caption'], text_data['caption_aug'], text_data['tokens'], text_data['tokens_aug']
        ...
        if len(tokens_aug) < self.opt.max_text_len:
            # pad with "unk"
            tokens = ['sos/OTHER'] + tokens_aug + ['eos/OTHER']
            sent_len = len(tokens)
            tokens = tokens + ['unk/OTHER'] * (self.opt.max_text_len + 2 - sent_len)
        else:
            # crop
            tokens = tokens_aug[:self.opt.max_text_len]
            tokens = ['sos/OTHER'] + tokens_aug + ['eos/OTHER']
            sent_len = len(tokens)
        # pos_one_hots = []
        # word_embeddings = []
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", **kwargs):
        ...
        opt.train_sample = True

        opt.text_dir = '/home/silijia/TMR-bone/dataset/KIT-ML/texts_aug_test_0.8'
        # print(opt.motion_dir)
        ...
        if mode == 'eval':
            ...
            opt.train_sample = False

        self.split_file = pjoin(opt.data_root, f'{split}.txt')
        self.train_sample = opt.train_sample
        ...
        if mode == 'text_only':
            ...
            self.t2m_dataset = Text2MotionDatasetV2(self.opt, self.mean, self.std, self.split_file, self.w_vectorizer, labels_file, randomize, self.train_sample)
